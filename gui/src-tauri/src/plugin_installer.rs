...
#[cfg(not(debug_assertions))]
const PLUGIN_INSTALLER_PATH: &str = "/usr/libexec/griffon-plugin-installer";
...
#[tauri::command]
pub async fn install_plugin_zip(zip_path: String) -> Result<InstalledPlugin, String> {
    tauri::async_runtime::spawn_blocking(move || install_plugin_zip_inner(zip_path))
        .await
        .map_err(|error| format!("Plugin installation task failed: {error}"))?
}

fn install_plugin_zip_inner(zip_path: String) -> Result<InstalledPlugin, String> {
    let zip_source = PathBuf::from(&zip_path);

    if !zip_source.is_file() {
        return Err(format!("ZIP file does not exist: {}", zip_source.display()));
    }
...
#[cfg(debug_assertions)]
fn install_plugin_zip_dev(zip_source: &Path) -> Result<InstalledPlugin, String> {
    ...
    let zip_file = fs::File::open(zip_source)
        .map_err(|error| format!("Failed to open ZIP file {}: {error}", zip_source.display()))?;
    ...
    for index in 0..archive.len() {
        let mut archive_file = archive
            .by_index(index)
            .map_err(|error| format!("Failed to access file in ZIP archive: {error}"))?;

        let enclosed_path = archive_file
            .enclosed_name()
            .ok_or_else(|| format!("Unsafe path found in ZIP archive at index {index}"))?
            .to_owned();
        ...
        if extension == "toml" && installed_toml.is_some() {
            return Err("Multiple TOML manifests found in archive".to_string());
        }

        if extension == "so" && installed_so.is_some() {
            return Err("Multiple shared libraries found in archive".to_string());
        }

        let file_name = enclosed_path
            .file_name()
            .ok_or_else(|| "Invalid file name in ZIP archive".to_string())?;
        ...
        io::copy(&mut archive_file, &mut output_file)
...
#[cfg(not(debug_assertions))]
fn install_plugin_zip_release(zip_source: &Path) -> Result<InstalledPlugin, String> {
    let canonical_archive = zip_source.canonicalize().map_err(|error| {
        format!(
    ...
    let output = Command::new("pkexec")
        ...
        .output()
        .map_err(|error| format!("Failed to start administrator authentication: {error}"))?;
    ...
    serde_json::from_slice::<InstalledPlugin>(&output.stdout).map_err(|error| {
...
#[tauri::command]
pub async fn delete_plugin(plugin_uuid: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || delete_plugin_inner(&plugin_uuid))
        .await
        .map_err(|error| format!("Plugin deletion task failed: {error}"))?
}

fn delete_plugin_inner(plugin_uuid: &str) -> Result<(), String> {
...
#[cfg(debug_assertions)]
fn delete_plugin_dev(plugin_uuid: &str) -> Result<(), String> {
    let plugin_dir = griffon_config_dir()?;

    let (manifest_path, library_path) = find_plugin_files_by_uuid(&plugin_dir, plugin_uuid)?;
...
fn find_plugin_files_by_uuid(
    ...
    let entries = fs::read_dir(&canonical_plugin_dir).map_err(|error| {
        format!(
    ...
    for entry in entries {
        ...
        let manifest: toml::Value = toml::from_str(&manifest_content).map_err(|error| {
            format!(
        ...
        if manifest_uuid.is_some_and(|uuid| uuid.eq_ignore_ascii_case(plugin_uuid)) {
    ...
    Err(format!("No plugin manifest found for UUID {plugin_uuid}"))
}
...
#[cfg(not(debug_assertions))]
fn delete_plugin_release(plugin_uuid: &str) -> Result<(), String> {
    let output = Command::new("pkexec")
        .arg(PLUGIN_INSTALLER_PATH)
        .arg("remove")
        .arg("--uuid")
...
#[cfg(not(debug_assertions))]
fn format_helper_error(status_code: Option<i32>, stderr: &[u8], operation: &str) -> String {
    ...
    match status_code {
        ...
        Some(127) => {
            format!("{operation} failed because administrator authorization was denied")
        }
