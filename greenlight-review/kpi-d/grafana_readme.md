...
The second method to instert data is from a json file directly generated by a plugin.
For example the default cleaner plugin has a test function that export a json file with some metrics in it.

To proccess this file you need to find the corresponding pyhton script, in this example it is `grafana/script/json_to_sqlite.py`, you can see it's usage by running it with no arguments.  
The script should transfer all the data from the json file to the sqlite database automatically.
