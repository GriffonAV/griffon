...
fn main() -> ExitCode {
    match run() {
        Ok(Some(installed_plugin)) => {
            match serde_json::to_string(&installed_plugin) {
                Ok(json) => {
                    println!("{json}");
                    ExitCode::SUCCESS
                }
                ...
                Err(error) => {
                    eprintln!(
                        "Failed to serialize installer response: {error}"
                    );
                    ExitCode::FAILURE
                }
            }
...
fn ensure_root() -> Result<(), String> {
    if unsafe { libc::geteuid() } != 0 {
        return Err(
            "griffon-plugin-installer must be executed with administrator privileges"
                .to_string(),
        );
    }
...
fn install_plugin_archive(
    ...
    let archive_path =
        archive_path.canonicalize().map_err(|error| {
            format!(
    ...
    let mut archive =
        zip::ZipArchive::new(zip_file).map_err(|error| {
            format!("Unable to read ZIP archive: {error}")
        })?;
    ...
    for index in 0..archive.len() {
        let mut archive_file =
            archive.by_index(index).map_err(|error| {
                format!(
                    "Unable to access ZIP entry {index}: {error}"
                )
            })?;
        ...
        let enclosed_path = archive_file
            .enclosed_name()
            .ok_or_else(|| {
                "Unsafe path found in ZIP archive".to_string()
            })?
            .to_owned();
        ...
        match extension.as_str() {
            "toml" if extracted_toml.is_some() => {
                return Err(
                    "Archive contains multiple TOML manifests"
                        .to_string(),
                );
            }
            ...
            "so" if extracted_so.is_some() => {
                return Err(
                    "Archive contains multiple shared libraries"
                        .to_string(),
                );
            }
        ...
        let file_name =
            enclosed_path.file_name().ok_or_else(|| {
                format!(
        ...
        let extracted_path =
            temporary_directory.path().join(file_name);

        let mut extracted_file =
            fs::File::create(&extracted_path).map_err(
                |error| {
                    format!(
        ...
        io::copy(&mut archive_file, &mut extracted_file).map_err(
            |error| {
                format!(
    ...
    let extracted_toml = extracted_toml.ok_or_else(|| {
        "Archive does not contain a TOML manifest".to_string()
    })?;

    let extracted_so = extracted_so.ok_or_else(|| {
        "Archive does not contain a shared library".to_string()
    })?;
    ...
    let toml_destination = plugin_directory.join(
        extracted_toml
            .file_name()
            .ok_or("Invalid TOML file name")?,
    );
    ...
    Ok(InstalledPlugin {
        toml_path: toml_destination
            .to_string_lossy()
            .into_owned(),

        so_path: so_destination
            .to_string_lossy()
            .into_owned(),
...
fn atomic_install(
    ...
    fs::copy(source, &temporary_destination).map_err(
        |error| {
            format!(
    ...
    fs::rename(&temporary_destination, destination).map_err(
        |error| {
            let _ = fs::remove_file(&temporary_destination);
...
fn remove_plugin_by_uuid(
    ...
    let canonical_plugin_directory =
        plugin_directory.canonicalize().map_err(|error| {
            format!(
     ...
     */
    remove_file_inside_plugin_directory(
        &library_path,
        &canonical_plugin_directory,
        false,
    )?;
...
fn find_plugin_files_by_uuid(
    ...
    let entries =
        fs::read_dir(plugin_directory).map_err(|error| {
            format!(
    ...
    for entry in entries {
        let entry = entry.map_err(|error| {
            format!(
                "Unable to read plugin directory entry: {error}"
            )
        })?;
        ...
        let canonical_manifest =
            path.canonicalize().map_err(|error| {
                format!(
                    "Unable to resolve manifest {}: {error}",
                    path.display()
                )
            })?;
        ...
        let Some(manifest_uuid) = find_uuid_in_toml(&manifest)
        else {
            continue;
        };

        let normalized_manifest_uuid =
            match Uuid::parse_str(manifest_uuid) {
    ...
    Err(format!(
        "No installed plugin found for UUID {plugin_uuid}"
    ))
}
...
fn remove_file_inside_plugin_directory(
    ...
    let canonical_path =
        path.canonicalize().map_err(|error| {
            format!(
                "Unable to resolve plugin file {}: {error}",
                path.display()
            )
        })?;
